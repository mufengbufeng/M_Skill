...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class EditorResourceComponent : MonoBehaviour, IResourceManager
    {
        ...
        [SerializeField] private float m_MinLoadAssetRandomDelaySeconds = 0f;

        [SerializeField] private float m_MaxLoadAssetRandomDelaySeconds = 0f;
        ...
        {
            get { throw new NotSupportedException("ReadWriteVersionListSerializer"); }
        }
        ...
        {
            get { throw new NotSupportedException("ResourcePackVersionListSerializer"); }
        }
        ...
        {
            get { throw new NotSupportedException("UpdateWaitingWhilePlayingCount"); }
        }
        ...
        {
            get { return m_LoadAssetInfos.Count; }
        }
...
#pragma warning restore 0067, 0414
                ...
                while (current != null && count < m_LoadAssetCountPerFrame)
                {
                    ...
                    if (elapseSeconds >= loadAssetInfo.DelaySeconds)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                loadAssetInfo.LoadAssetCallbacks.LoadAssetFailureCallback(loadAssetInfo.AssetName, LoadResourceStatus.AssetError,
                                    "Can not load this asset from asset database.", loadAssetInfo.UserData);
                            }
                        }
                        ...
                        if (loadSceneInfo.AsyncOperation.allowSceneActivation)
                        {
                            if (loadSceneInfo.LoadSceneCallbacks.LoadSceneSuccessCallback != null)
                            {
                                loadSceneInfo.LoadSceneCallbacks.LoadSceneSuccessCallback(loadSceneInfo.SceneAssetName,
                                    (float)(DateTime.UtcNow - loadSceneInfo.StartTime).TotalSeconds, loadSceneInfo.UserData);
                            }
                        }
                        else
                        {
                            ...
                            {
                                loadSceneInfo.LoadSceneCallbacks.LoadSceneFailureCallback(loadSceneInfo.SceneAssetName, LoadResourceStatus.AssetError,
                                    "Can not load this scene from asset database.", loadSceneInfo.UserData);
                            }
                        }
        ...
        public void LoadScene(string sceneAssetName, int priority, LoadSceneCallbacks loadSceneCallbacks, object userData)
        {
            ...
            if (!HasFile(sceneAssetName))
            {
                if (loadSceneCallbacks.LoadSceneFailureCallback != null)
                {
                    loadSceneCallbacks.LoadSceneFailureCallback(sceneAssetName, LoadResourceStatus.NotExist, Utility.Text.Format("Scene '{0}' is not exist.", sceneAssetName),
                        userData);
                }
