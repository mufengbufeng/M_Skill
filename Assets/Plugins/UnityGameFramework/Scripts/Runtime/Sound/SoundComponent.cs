...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed partial class SoundComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            ...
            {
                if (!AddSoundGroup(m_SoundGroups[i].Name, m_SoundGroups[i].AvoidBeingReplacedBySamePriority, m_SoundGroups[i].Mute, m_SoundGroups[i].Volume,
                        m_SoundGroups[i].AgentHelperCount))
                {
        ...
        {
            string logMessage = Utility.Text.Format("Play sound failure, asset name '{0}', sound group name '{1}', error code '{2}', error message '{3}'.", e.SoundAssetName,
                e.SoundGroupName, e.ErrorCode, e.ErrorMessage);
