...
public static class UIExtension
{
    ...
    public static void LoadSpriteAtlas(this UIComponent uiCom, string atlasName, GameFrameworkAction<SpriteAtlas> onSpriteAtlasLoaded)
    {
        ...
        {
            Log.Warning($"LoadSpriteAtlas失败, 资源不存在:{atlasName}");
            return;
        }
    ...
    public static void LoadSprite(this UIComponent uiCom, string spriteName, GameFrameworkAction<Sprite> onSpriteLoaded)
    {
        ...
        {
            Log.Warning($"UIExtension.SetSprite()失败, 资源不存在:{spriteName}");
            return;
        }
    ...
    {
        int coinNum = num;
        DOVirtual.DelayedCall(flyDelay, () => { GF.Sound.PlayEffect("add_money.wav"); });
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            animPrams.OnShowCallback = moneyEntity =>
            {
                ...
                var expDuration = Vector2.Distance(moneyEntity.transform.position, expPos) *
                                  0.05f; // Mathf.Clamp(Vector3.Distance(moneyEntity.transform.position, expPos)*0.01f, 0.1f, 0.4f);
