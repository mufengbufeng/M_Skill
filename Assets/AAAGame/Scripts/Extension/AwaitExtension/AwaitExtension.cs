...
public static class AwaitExtension
{
    ...
    private static void OnLoadDataTableSuccess(object sender, GameEventArgs e)
    {
        ...
        if (tcs != null)
        {
            Log.Info($"Load data table '{ne.DataTableAssetName}' OK.");
    ...
    private static void OnLoadDataTableFailure(object sender, GameEventArgs e)
    {
        ...
        if (tcs != null)
        {
            Log.Error($"Can not load data table '{ne.DataTableAssetName}' from '{ne.DataTableAssetName}' with error message '{ne.ErrorMessage}'.");
    ...
    /// 打开界面（可等待）
    /// </summary>
    public static UniTask<UIFormLogic> OpenUIFormAwait(this UIComponent uiComponent, string uiFormAssetName, string uiGroupName, int priority, bool pauseCoveredUIForm,
        object userData)
    {
#if UNITY_EDITOR
        TipsSubscribeEvent();
