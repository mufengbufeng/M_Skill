...
namespace GameFramework.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        {
            return new DataTableProcessor(Utility.Path.GetRegularPath(Path.Combine(ConstEditor.DataTablePath, dataTableName + ".txt")), Encoding.Unicode, 1, 2, null, 3, 4,
        ...
        public static void GenerateCodeFile(DataTableProcessor dataTableProcessor, string dataTableName)
        {
            ...
            {
                Log.Error(Utility.Text.Format("生成{0}数据表结构代码失败:{1}", dataTableName, csharpCodeFileName));
            }
        }
        ...
        private static string GenerateDataTableProperties(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTypeKeyword == "enum")
                {
                    ...
                    if (!TryParseEnum(firstEnumValue, out Type enumType))
                    {
                        Log.Error(Utility.Text.Format("解析枚举类型失败:{0}, 配置枚举格式为: EnumType.Item1", firstEnumValue));
                        continue;
                    }
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTableProcessor.IsSystem(i))
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseArray<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[]", string.Empty)).AppendLine();
                        }
                        else if (isArrayType == 2)
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse2DArray<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[][]", string.Empty)).AppendLine();
                        }
                    }
                    else
                    {
                        ...
                        else if (languageKeyword == "enum")
                        {
                            ...
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseEnum<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i),
                                enumType.FullName).AppendLine();
                        }
                        else
                        {
                ...
                else
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}Array(columnStrings[index++]);", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[]", string.Empty)).AppendLine();
                        }
                        else if (isArrayType == 2)
                        {
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}2DArray(columnStrings[index++]);", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[][]", string.Empty)).AppendLine();
                        }
                    }
                    else
                    {
                        stringBuilder.AppendFormat("            {0} = DataTableExtension.Parse{1}(columnStrings[index++]);", dataTableProcessor.GetName(i),
                            dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                }
            }
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                if (dataTableProcessor.IsSystem(i))
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            stringBuilder.AppendFormat("                    {0} = DataTableExtension.ParseArray<{1}>(binaryReader.ReadString());", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[]", string.Empty)).AppendLine();
                        }
                        else if (isArrayType == 2)
                        {
                            stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse2DArray<{1}>(binaryReader.ReadString());", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[][]", string.Empty)).AppendLine();
                        }
                    }
                    else
                    {
                        ...
                        {
                            stringBuilder.AppendFormat("                    {0} = binaryReader.Read7BitEncoded{1}();", dataTableProcessor.GetName(i),
                                dataTableProcessor.GetType(i).Name).AppendLine();
                        }
                        else if (languageKeyword == "enum")
                        {
                            ...
                            stringBuilder.AppendFormat("                    {0} = ({1})binaryReader.Read7BitEncodedInt32();", dataTableProcessor.GetName(i), enumType.FullName)
                                .AppendLine();
                        }
                        else
                        {
                ...
                else
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse{1}Array(binaryReader.ReadString());", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[]", string.Empty)).AppendLine();
                        }
                        else if (isArrayType == 2)
                        {
                            stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse{1}2DArray(binaryReader.ReadString());", dataTableProcessor.GetName(i),
                                languageKeyword.Replace("[][]", string.Empty)).AppendLine();
                        }
                    }
                    else
                    {
                        stringBuilder.AppendFormat("                    {0} = DataTableExtension.Parse{1}(binaryReader.ReadString());", dataTableProcessor.GetName(i),
                            dataTableProcessor.GetType(i).Name).AppendLine();
                    }
                }
            }
        ...
        private static bool TryParseEnum(string enumValue, out Type enumType)
        {
            enumType = null;
