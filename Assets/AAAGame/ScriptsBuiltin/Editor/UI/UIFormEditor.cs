    ...
    public class UIFormBaseEditor : UnityEditor.Editor
    {
        ...
        static void InitEditor()
        {
            ...
            EditorApplication.hierarchyWindowItemOnGUI = delegate(int id, Rect rect)
            {
                ...
                if (drawItem != null)
                {
                    ...
                    var displayContent = EditorGUIUtility.TrTextContent(Utility.Text.Format("{0} {1} {2}", GetVarPrefix(drawItem.VarPrefix),
                        GetDisplayVarTypeName(drawItem.VarType), drawItem.VarName));
        ...
        private static void AddClickButtonEvent<T>()
        {
            ...
            foreach (var item in Selection.gameObjects)
            {
                ...
                var m_OnClick =
                    buttonCom.GetType().GetField("m_OnClick", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).GetValue(buttonCom) as UnityEvent;
        ...
        private static void OpenSelectComponentMenuListener(Rect rect)
        {
            if (mShowSelectTypeMenu)
            {
                ...
                EditorUtility.DisplayCustomMenu(rect, contents, idx, (userData, contents, selected) => { AddToFields(varPrefixIndex, contents[selected]); }, null);
                mShowSelectTypeMenu = false;
            }
        }
