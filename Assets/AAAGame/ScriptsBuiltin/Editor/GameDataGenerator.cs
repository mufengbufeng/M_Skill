...
namespace UGF.EditorTools
{
    ...
    public class GameDataGenerator
    {
        ...
        private static List<KeyValuePair<int, string>> ScanVariableTypes()
        {
            ...
            var nestedTypes = typeof(GameFramework.Editor.DataTableTools.DataTableProcessor).GetNestedTypes(System.Reflection.BindingFlags.NonPublic |
        ...
        public static void RefreshAllLanguage(IList<string> files = null)
        {
            ...
            for (int i = 0; i < totalExcelCount; i++)
            {
                ...
                {
                    Log.Info($"Language导出成功:{outputFileName}");
                }
            }
        ...
        public static void RefreshAllConfig(IList<string> files = null)
        {
            ...
            for (int i = 0; i < totalExcelCount; i++)
            {
                ...
                {
                    Log.Info($"导出Config成功:{outputFileName}");
                }
            }
        ...
        public static void RefreshAllDataTable(IList<string> fullPathFiles = null)
        {
            ...
            for (int i = 0; i < totalExcelCount; i++)
            {
                ...
                try
                {
                    ...
                    {
                        Log.Info($"导出DataTable成功:{excelFileName} -> {outputPath}");
                        //if (appConfig.DataTableBytesMode)
                        //{
        ...
        public static string GetGameDataExcelRelativePath(GameDataType tp, string excelFile)
        {
            ...
            excelRelativePath =
                UtilityBuiltin.AssetsPath.GetCombinePath(Path.GetDirectoryName(excelRelativePath), Path.GetFileNameWithoutExtension(excelRelativePath)); // 获取表的相对路径并去掉扩展名
