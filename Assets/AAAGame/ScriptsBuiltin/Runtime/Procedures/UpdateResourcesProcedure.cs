...
public class UpdateResourcesProcedure : ProcedureBase
{
    ...
    private long mDownloadTotalZipLength;
    ...
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        ...
        GFBuiltin.Event.Subscribe(ResourceUpdateStartEventArgs.EventId, OnResourceUpdateStart);
        GFBuiltin.Event.Subscribe(ResourceUpdateChangedEventArgs.EventId, OnResourceUpdateChanged);
        GFBuiltin.Event.Subscribe(ResourceUpdateSuccessEventArgs.EventId, OnResourceUpdateSuccess);
        GFBuiltin.Event.Subscribe(ResourceUpdateAllCompleteEventArgs.EventId, OnResourceUpdateAllComplete);
        GFBuiltin.Event.Subscribe(ResourceUpdateFailureEventArgs.EventId, OnResourceUpdateFailure);
        GFBuiltin.Event.Subscribe(ResourceVerifyStartEventArgs.EventId, OnResourceVerifyStart);
        GFBuiltin.Event.Subscribe(ResourceVerifySuccessEventArgs.EventId, OnResourceVerifySuccess);
        GFBuiltin.Event.Subscribe(ResourceVerifyFailureEventArgs.EventId, OnResourceVerifyFailure);
        CheckVersion();
    }
    ...
    protected override void OnLeave(IFsm<IProcedureManager> procedureOwner, bool isShutdown)
    {
        ...
        GFBuiltin.Event.Unsubscribe(ResourceUpdateStartEventArgs.EventId, OnResourceUpdateStart);
        GFBuiltin.Event.Unsubscribe(ResourceUpdateChangedEventArgs.EventId, OnResourceUpdateChanged);
        GFBuiltin.Event.Unsubscribe(ResourceUpdateSuccessEventArgs.EventId, OnResourceUpdateSuccess);
        GFBuiltin.Event.Unsubscribe(ResourceUpdateAllCompleteEventArgs.EventId, OnResourceUpdateAllComplete);
        GFBuiltin.Event.Unsubscribe(ResourceUpdateFailureEventArgs.EventId, OnResourceUpdateFailure);
        GFBuiltin.Event.Unsubscribe(ResourceVerifyStartEventArgs.EventId, OnResourceVerifyStart);
        GFBuiltin.Event.Unsubscribe(ResourceVerifySuccessEventArgs.EventId, OnResourceVerifySuccess);
        GFBuiltin.Event.Unsubscribe(ResourceVerifyFailureEventArgs.EventId, OnResourceVerifyFailure);
    ...
    void CheckVersion()
    {
        if (GFBuiltin.Resource.ResourceMode == ResourceMode.Updatable || GFBuiltin.Resource.ResourceMode == ResourceMode.UpdatableWhilePlaying)
        {
    ...
    private void CheckVersionList(VersionInfo vinfo)
    {
        if (vinfo == null)
        {
            Log.Fatal("热更失败: 解析version.json信息失败!");
            return;
        }
        ...
        var curAppVersion = Version.Parse(GameFramework.Version.GameVersion);
        var lastAppVersion = Version.Parse(vinfo.LastAppVersion);
    ...
    private void CheckVersionAndUpdate(VersionInfo vinfo)
    {
        ...
        Log.Info($"资源服务器地址:{GFBuiltin.Resource.UpdatePrefixUri}");
        ...
        if (checkResult == CheckVersionListResult.NeedUpdate)
        {
            Log.Info("更新资源列表文件...");
            var updateVersionCall = new UpdateVersionListCallbacks(OnUpdateVersionListSuccess, OnUpdateVersionListFailed);
            GFBuiltin.Resource.UpdateVersionList(vinfo.VersionListLength, vinfo.VersionListHashCode, vinfo.VersionListCompressedLength, vinfo.VersionListCompressedHashCode,
                updateVersionCall);
        }
        else
        {
    ...
    private void OnCheckResurcesComplete(int movedCount, int removedCount, int updateCount, long updateTotalLength, long updateTotalZipLength)
    {
        ...
        {
            Log.Info($"需要更新资源个数:{updateCount},资源大小:{updateTotalZipLength},下载地址:{GFBuiltin.Resource.UpdatePrefixUri}");
    ...
    private void OnResourceUpdateFailure(object sender, GameEventArgs e)
    {
        ...
        {
            Log.Error($"Download '{ne.Name}' failure from '{ne.DownloadUri}' with error message '{ne.ErrorMessage}', retry count '{ne.RetryCount}'.");
            return;
        }

        Log.Warning($"Download '{ne.Name}' failure from '{ne.DownloadUri}' with error message '{ne.ErrorMessage}', retry count '{ne.RetryCount}'.");
    ...
    private void OnResourceUpdateSuccess(object sender, GameEventArgs e)
    {
        ...
        Log.Info($"Download '{ne.Name}' success.");
    ...
    {
        Log.Fatal($"UpdateVersionListFailed, downloadUri:{downloadUri}, errorMessage:{errorMessage}");
    }
    ...
    private void OnResourceVerifyStart(object sender, GameEventArgs e)
    {
        ...
        Log.Info($"Start verify resources, verify resource count '{ne.Count}', verify resource total length '{ne.TotalLength}'.");
    }
    ...
    private void OnResourceVerifySuccess(object sender, GameEventArgs e)
    {
        ...
        Log.Info($"Verify resource '{ne.Name}' success.");
    }
    ...
    private void OnResourceVerifyFailure(object sender, GameEventArgs e)
    {
        ...
        Log.Warning($"Verify resource '{ne.Name}' failure.");
    }
